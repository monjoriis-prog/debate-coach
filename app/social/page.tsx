...
export default function Forte() {
  ...
  // SCENARIO PICKER
  if (phase === "scenario") {
    const subcategories: string[] = selectedCategory.situations
      .map((s: any) => s.subcategory)
      .filter(Boolean)
      .filter((v: string, i: number, a: string[]) => a.indexOf(v) === i);
    const hasSubcategories = subcategories.length > 0;
    const filtered = hasSubcategories && subcategoryFilter !== "All"
      ? selectedCategory.situations.filter((s: any) => s.subcategory === subcategoryFilter)
      : selectedCategory.situations;
    return (
    <div style={{ minHeight: "100vh", background: "#f8faf8", fontFamily: "Georgia, serif" }}>
      <div style={{ maxWidth: "600px", margin: "0 auto", padding: "48px 24px" }}>
        <button onClick={() => { setPhase("home"); setSubcategoryFilter("All"); }} style={{ background: "transparent", border: "none", color: "#84a98c", cursor: "pointer", fontSize: "14px", marginBottom: "40px", padding: 0, fontFamily: "-apple-system, sans-serif" }}>← Back</button>
        ...
        <p style={{ color: "#84a98c", fontSize: "14px", marginBottom: hasSubcategories ? "20px" : "32px", fontFamily: "-apple-system, sans-serif" }}>Choose a scenario — you'll learn first, then practice</p>

        {hasSubcategories && (
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "28px" }}>
            {["All", ...subcategories].map((sub) => (
              <button key={sub} onClick={() => setSubcategoryFilter(sub)}
                style={{ padding: "7px 16px", borderRadius: "99px", border: `1.5px solid ${subcategoryFilter === sub ? selectedCategory.accent : "#d8e8e0"}`, background: subcategoryFilter === sub ? selectedCategory.accent : "#fff", color: subcategoryFilter === sub ? "#fff" : "#52796f", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: "-apple-system, sans-serif", transition: "all 0.2s" }}>
                {sub}
              </button>
            ))}
          </div>
          ...
          {filtered.map((s: any, i: number) => (
            <button key={i} onClick={() => { setSelectedSituation(s); setLessonIndex(0); setPhase("learn"); }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  {s.subcategory && subcategoryFilter === "All" && (
                    <div style={{ fontSize: "10px", fontWeight: "700", color: selectedCategory.accent, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "6px", fontFamily: "-apple-system, sans-serif" }}>{s.subcategory}</div>
                  )}
