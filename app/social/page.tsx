...
const SCENARIOS = [
  ...
  },
  {
    ...
    emoji: "👨‍👩‍👧",
    situations: [
      { title: "Talking to a teenager", prompt: "You're a 16-year-old who's been distant. Give short answers, seem distracted, but open up slightly if they're genuine and not pushy.", ai_role: "your teenager", voice: { pitch: 1.3, rate: 1.15, preferFemale: false } },
      { title: "Setting limits with a parent", prompt: "You're a well-meaning parent who gives too much advice. React with slight hurt if pushed away harshly, but respect firm kindness.", ai_role: "your parent", voice: { pitch: 0.9, rate: 0.9, preferFemale: false } },
      { title: "Comforting a young child", prompt: "You're a 7-year-old upset because a friend was mean. Speak simply, emotionally, and respond to warmth.", ai_role: "your child", voice: { pitch: 1.6, rate: 1.1, preferFemale: true } },
    ]
  },
  {
    category: "Work",
    emoji: "💼",
    situations: [
      { title: "Asking for a raise", prompt: "You're a busy manager. Start noncommittal, ask for justification. Respond positively to confident, evidence-based arguments.", ai_role: "your boss", voice: { pitch: 0.8, rate: 0.88, preferFemale: false } },
      { title: "Handling a difficult coworker", prompt: "You've been taking credit for others' work without realizing it fully. Be defensive at first but not evil — just oblivious.", ai_role: "your coworker", voice: { pitch: 1.0, rate: 1.05, preferFemale: true } },
      { title: "Networking at an event", prompt: "You're a professional at a networking event. Be friendly but slightly guarded. Open up if they're interesting and genuine.", ai_role: "a professional contact", voice: { pitch: 0.95, rate: 1.0, preferFemale: false } },
    ]
  },
  {
    category: "Social",
    emoji: "🤝",
    situations: [
      { title: "Meeting a new neighbor", prompt: "You're a neighbor meeting someone new. Be friendly and a bit curious. Ask about where they moved from.", ai_role: "your neighbor", voice: { pitch: 1.05, rate: 1.0, preferFemale: true } },
      { title: "Reconnecting with an old friend", prompt: "You're an old friend who felt hurt by the distance. Be warm but slightly guarded. Open up if they acknowledge the gap.", ai_role: "your old friend", voice: { pitch: 1.1, rate: 0.98, preferFemale: false } },
      { title: "Making small talk", prompt: "You're a friendly stranger in a queue. Keep things light, respond to what they say, keep it natural.", ai_role: "a friendly stranger", voice: { pitch: 1.0, rate: 1.05, preferFemale: true } },
    ]
  },
];
...
Rules:
1. Stay completely in character as ${aiRole}. Be realistic — add natural friction and emotion.
...
function parseFeedback(text) {
  ...
  return {
    warmth: parseInt(get("WARMTH")),
    clarity: parseInt(get("CLARITY")),
    listening: parseInt(get("LISTENING")),
    confidence: parseInt(get("CONFIDENCE")),
...
export default function SocialCoach() {
  ...
  async function startChat(situation) {
  ...
  async function sendMessage(text) {
    ...
    if (parsed) {
      ...
      setPhase("done");
      setSpeaking(true);
      speak(parsed.verdict, { pitch: 1.0, rate: 0.92, preferFemale: false }, () => setSpeaking(false));
    } else {
      ...
      <div style={{ minHeight: "100vh", background: "#0a0a1a", color: "#f0f0f0", fontFamily: "Georgia, serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
        <div style={{ maxWidth: "560px", width: "100%" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {selectedCategory.situations.map((s, i) => (
        ...
        {phase === "done" && feedback && (
          <div style={{ border: "1px solid #2d2d5e", background: "#090918", padding: "28px", borderRadius: "4px", marginTop: "12px" }}>
            ...
            <div style={{ marginTop: "12px", padding: "16px", background: "#0d0d20", border: "1px solid #1e1e3f", borderRadius: "4px" }}>
              <div style={{ fontSize: "11px", color: "#666", textTransform: "uppercase", marginBottom: "8px" }}>Coach's Assessment</div>
      ...
      {phase === "chat" && (
        <div style={{ borderTop: "1px solid #1a1a2e", padding: "24px", display: "flex", flexDirection: "column", alignItems: "center", gap: "12px", flexShrink: 0 }}>
          ...
          <div style={{ fontSize: "11px", color: "#444", letterSpacing: "0.15em", textTransform: "uppercase" }}>
            {listening ? "Release to send" : loading ? "Thinking..." : speaking ? "They're speaking..." : "Hold to speak"}
          </div>
        </div>
      )}
